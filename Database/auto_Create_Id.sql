...
BEGIN
	DECLARE @id VARCHAR(16)
	IF (SELECT COUNT(MaVeChuyenBay) FROM VECHUYENBAY) = 0
		SET @id = 0
	ELSE
		SELECT @id = MAX(RIGHT(MaVeChuyenBay, 5)) FROM VECHUYENBAY
		SELECT @id = CASE
...
ALTER TABLE VECHUYENBAY ADD CONSTRAINT IDVCB DEFAULT(dbo.AUTO_IDVCB()) FOR MaVeChuyenBay
